...
table = []
with open('Production_allemagne.csv', newline='', encoding='latin-1') as csvfile:
...
del table[0]  # supprimer l'en-tête
 
production = {}
 
for i in range(len(table)):
    # Vérifier que la ligne a au moins 4 colonnes (pour éviter les erreurs)
    if len(table[i]) < 4:
        continue
...
plt.title("Production d'électricité en Allemagne")
plt.legend()
plt.grid()
plt.show()
